...
// 🟢 订阅处理逻辑
// =============================================================================
function isSubPath(pw, url) {
 return pw && url.pathname === `/${pw}`;
}

function isNormalSub(uuid, url) {
 return url.pathname === '/sub' && url.searchParams.get('uuid') === uuid;
}
 ...
 async fetch(r, env, ctx) {
   try {
     ...
     if (url.pathname === '/admin' || url.pathname === '/admin/') {
       ...
       return new Response(dashPage(host, _UUID, _PROXY_IP, _SUB_PW, _POOL_DOMAINS, currentIndex, nodeCount), {
         status: 200,
